...
The app uses Expo Router to implement navigation, taking advantage of its file-based routing system for a clean and modular architecture. The top-level `app/` directory contains the main layout and entry screens, including `_layout.tsx` and `login.tsx` as well as a grouped folder `(tabs)` that manages the tab-based navigation structure. Inside the `(tabs)` directory, screens such as `index.tsx` (Home Screen), `newRecipe.tsx` (Add Recipe Screen), `personal.tsx` (User Profile), `personalCollection.tsx` (User Collection Screen), and `recipeDetails.tsx` (Recipe Details Screen) are organized with their own shared `_layout.tsx` to define consistent tab navigation and UI styling. Navigation between screens is handled declaratively using the Expo Router router object, enabling smooth transitions and data passing through route parameters (e.g., sending a selected recipe ID from the Home Screen to the Recipe Details Screen). Shared layouts can be defined in `app/_layout.tsx` to include consistent UI components such as headers, navigation bars, or styling wrappers across multiple screens.
